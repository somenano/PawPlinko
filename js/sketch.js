...
function preload() {
  try {
    bumper_dot = loadImage('images/bumper_dot.png');
  }
  catch (err) {
    console.error(err);
  }
...
function newParticle(address) {
  ...
  if (Object.keys(pawnimal_images).indexOf(address) != -1) {
    ...
    // console.log('Pawnimal not found in cache, loading: '+ address);
    const path = get_pawnimal(address, 'svg');
    loadImage(path, function(img) {
      ...
      try {
        ...
        loadImage(get_pawnimal(address, 'png'), function(png) {
          pawnimal_images[address] = png;
          newParticle(address);
        });
        return;
      }
      pawnimal_images[address] = img;
      newParticle(address);
    }, function(event) {
...
function drop_test_pawnimal() {
  // This simply drops the binance cold wallet nano address as pawnimal
  newParticle('paw_3x4ui45q1cw8hydmfdn4ec5ijsdqi4ryp14g4ayh71jcdkwmddrq7ca9xzn9');
}
