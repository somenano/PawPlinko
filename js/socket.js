...
new_websocket('wss://pawnode.somenano.com/repeater', function(socket) {
    // onopen
    let params = {
        action: 'subscribe',
        topic: 'confirmation'
...
function handle_block_dump(data)
{
    ...
    console.log(''+ String(dtg.getHours()).padStart(2, '0') +':'+ String(dtg.getMinutes()).padStart(2, '0') +':'+ String(dtg.getSeconds()).padStart(2, '0') + ' - Received '+ blocks.length +' Paw Blocks from the last '+ (duration/1000).toFixed(2) +' second(s). CPS is '+ cps.toFixed(2) +' over the last 30 seconds.');
